...
class HistoryScreen(ctk.CTkFrame):
    ...
    def __init__(self, parent, ui_manager, data_dir):
        super().__init__(parent, fg_color=("#F8F9FA","#0a0e27"))
    ...
    def load_data(self):
        ...
        try:
            ...
            print ("عندك مشكله في تحميل البيانات من Google Sheets")
    ...
    def fetch_history(self):
        ...
        # إذا كان في عمود تاريخ (Date أو Timestamp أو أي اسم)
        date_columns = [col for col in df.columns if 'date' in col.lower() or 'تاريخ' in col or 'time' in col.lower()]
        
        if date_columns:
            date_col = date_columns[0]  # نأخذ أول عمود يحتوي على تاريخ
            try:
                df[date_col] = pd.to_datetime(df[date_col], errors='coerce')
                df = df.sort_values(by=date_col, ascending=False).reset_index(drop=True)
            except:
                pass  # لو فشل التحويل، خليه كما هو
        
        return df

    def update_table(self, page=0):
        """عرض البيانات في الجدول مع تقسيم الصفحات واختيار عدد الصفوف في الصفحة"""
        ...
        page_data = self.filtered_df.iloc[start:end]

        # ضبط الأعمدة
        self.tree["columns"] = list(page_data.columns)
        ...
        # إزالة إطار التصفح القديم
        if hasattr(self, "pagination_frame") and self.pagination_frame.winfo_exists():
            self.pagination_frame.destroy()

        # إنشاء إطار جديد أسفل الجدول
        self.pagination_frame = ctk.CTkFrame(self, fg_color="transparent")
        self.pagination_frame.pack(side="bottom", pady=15, fill="x")

        # الإطار الداخلي — ترتيب العناصر في المنتصف
        self.pagination_inner = ctk.CTkFrame(self.pagination_frame, fg_color="transparent")
        self.pagination_inner.pack(anchor="center")

        # زر السابق
        self.prev_btn = ctk.CTkButton(
            self.pagination_inner, text="⬅️ السابق",
            width=100, height=35, corner_radius=8,
            command=lambda: self.change_page(-1),
            fg_color="#00b8d4", hover_color="#00a0b4",
            text_color="#0a0e27", font=("Segoe UI", 12, "bold")
        )
        ...
        self.page_label = ctk.CTkLabel(
            ...
            font=("Segoe UI", 13, "bold"), text_color="#00d9ff"
        )
        self.page_label.pack(side="left", padx=10)

        # زر التالي
        self.next_btn = ctk.CTkButton(
            self.pagination_inner, text="التالي ➡️",
            width=100, height=35, corner_radius=8,
            command=lambda: self.change_page(1),
            fg_color="#00b8d4", hover_color="#00a0b4",
            text_color="#0a0e27", font=("Segoe UI", 12, "bold")
        ...
        # 🔹 قائمة اختيار عدد الصفوف
        self.rows_per_page_label = ctk.CTkLabel(
            self.pagination_inner, text="صفوف/صفحة:", text_color="#00d9ff", font=("Segoe UI", 12)
        )
        self.rows_per_page_label.pack(side="left", padx=(30, 5))

        self.rows_per_page_dropdown = ctk.CTkOptionMenu(
            self.pagination_inner,
            values=["25", "50", "100", "200"],
            command=self.change_page_size,
            fg_color="#1f6aa5", button_color="#144870", text_color="white",
            dropdown_hover_color="#00b8d4"
        )
        self.rows_per_page_dropdown.set(str(self.page_size))
        self.rows_per_page_dropdown.pack(side="left", padx=5)

        # تفعيل/تعطيل الأزرار
        self.prev_btn.configure(state="normal" if page > 0 else "disabled")
        self.next_btn.configure(state="normal" if end < total_rows else "disabled")

    def change_page(self, direction):
        """التنقل بين الصفحات"""
        new_page = self.current_page + direction
        self.update_table(new_page)
