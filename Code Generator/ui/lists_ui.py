...
class ListsMixin:
        ...
        def confirm_delete():
            try:
                ...
                if saved:
                    ...
                    # ✅ حذف من Google Sheets أيضًا
                    if hasattr(self, "sync_manager") and self.sync_manager:
                        def delete_from_google_sheets():
                            try:
                                # استدعاء الدالة الخاصة بالحذف من الشيت
                                if hasattr(self.sync_manager, "delete_list_from_sheet"):
                                    self.sync_manager.delete_list_from_sheet(list_name)
                                    print(f"🧩 تم حذف '{list_name}' من Google Sheets.")
                                else:
                                    # fallback لو الدالة مش موجودة
                                ...
                                self._show_modern_warning("مزامنة Google Sheets", f"حدث خطأ أثناء حذف القائمة من Google Sheets:\n{e}")

                        threading.Thread(target=delete_from_google_sheets, daemon=True).start()

                    self._show_modern_success("تم الحذف", f"تم حذف القائمة\n'{list_name}' بنجاح ✅")
                    confirm_dialog.after(500, confirm_dialog.destroy)
                else:
    ...
    def show_add_selected_to_list_dialog(self):
        ...
        def add_and_save():
            ...
            # ✅ حذف المنتج من Google Sheets أيضًا (من عمود items داخل القائمة)
            if hasattr(self, "sync_manager") and self.sync_manager:
                product_code = self.get_final_code(product)
                threading.Thread(
                    target=self.sync_manager.delete_product_from_list_in_sheet,
                    args=(list_name, product_code),
                    daemon=True
                ).start()
                #print(f"🧩 تم إرسال طلب حذف المنتج '{product_code}' من القائمة '{list_name}' داخل Google Sheets.")
