...
class PrinterMixin:
    ...
    def generate_and_print_barcode(self, code, copies=1, width_inches=1.97, height_inches=0.98):
    ...
    def show_print_dialog(self, code):
        ...
        PAPER_SIZES = {
            "A4": (8.27, 11.69),
            "A5": (5.83, 8.27),
            "Barcode Sticker": (1.97, 0.98)
        }


        ctk.CTkLabel(dialog, text="نوع الورق:", font=("Arial", 12)).pack(pady=5)
        paper_var = ctk.StringVar(value="Barcode Sticker")
        paper_menu = ctk.CTkOptionMenu(dialog, variable=paper_var, values=list(PAPER_SIZES.keys()), width=200)
        paper_menu.pack(pady=5)
        ...
        def start_printing():
            try:
                ...
                paper_type = paper_var.get()
                width_inches, height_inches = PAPER_SIZES[paper_type]

                self.generate_and_print_barcode(
                    code,
                    copies=copies,
                    width_inches=width_inches,
                    height_inches=height_inches
